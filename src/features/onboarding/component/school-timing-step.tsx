...
export const SchoolTimingStep = ({
  onSubmit,
}: {
  onSubmit: (start: number, end: number) => void;
}) => {
  const [range, setRange] = useState<number[]>([8, 14]); // 8 AM to 2 PM default
  ...
  return (
    <div className="flex flex-col gap-6 w-full mt-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted-foreground">School Hours</span>
